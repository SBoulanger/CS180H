...
def run_MNIST_network():
    ...
    y = tf.placeholder(tf.float32, None)#confused on the None

    #create conv layers
    layer1 = create_new_conv_layer(input_data=x_shaped, num_input_channels=1,
                                  num_filters=32,filter_shape=[5,5],pool_shape=[2,2],name='ConvLayer1') 
    layer2 = create_new_conv_layer(input_data=layer1, num_input_channels=32,
                                  num_filters=64,filter_shape=[5,5],pool_shape=[2,2],name='ConvLayer2') 
    ...
    with tf.name_scope("DenseLayer2"):
